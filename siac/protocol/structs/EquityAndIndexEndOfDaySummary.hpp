...
#include "../types/EquityAndIndexEndOfDaySummaryMessageType.hpp"
...
struct EquityAndIndexEndOfDaySummary {
    ...
    // parse method
    static EquityAndIndexEndOfDaySummary* parse(std::byte* buffer) {
        return reinterpret_cast<EquityAndIndexEndOfDaySummary*>(buffer);
    }

    // parse method const
    static const EquityAndIndexEndOfDaySummary* parse(const std::byte* buffer) {
        return reinterpret_cast<const EquityAndIndexEndOfDaySummary*>(buffer);
    }
};
