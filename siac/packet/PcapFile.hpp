...
struct PcapFile : PcapSource {
    ...
    explicit PcapFile(const std::string& path,
        ...
        : buffer(open_decompressor(path), buf_size) {
        parse_file_header();
    }
    ...
    bool advance() override {
        ...
        return is_pcapng ? advance_pcapng() : advance_classic();
    }

    std::uint64_t timestamp_ns() const override { return ts_ns; }
    const std::byte* data() const override { return pkt_data; }
    std::uint32_t length() const override { return pkt_len; }
...
private:
    ...
    const std::byte* pkt_data = nullptr;
    ...
    // --- Endian helpers ---
    std::uint16_t fix16(std::uint16_t v) const { return swap_bytes ? std::byteswap(v) : v; }
    std::uint32_t fix32(std::uint32_t v) const { return swap_bytes ? std::byteswap(v) : v; }
    std::uint64_t fix64(std::uint64_t v) const { return swap_bytes ? std::byteswap(v) : v; }
    ...
    void parse_classic_header(std::uint32_t magic) {
        switch (magic) {
        case 0xa1b2c3d4: swap_bytes = false; nanosecond_ts = false; break;
        case 0xd4c3b2a1: swap_bytes = true;  nanosecond_ts = false; break;
        case 0xa1b23c4d: swap_bytes = false; nanosecond_ts = true;  break;
        case 0x4d3cb2a1: swap_bytes = true;  nanosecond_ts = true;  break;
        default:
    ...
    bool advance_classic() {
        if (!buffer.ensure(16)) { is_done = true; return false; }

        std::uint32_t ts_sec, ts_usec, caplen;
        std::memcpy(&ts_sec,  buffer.data(),      4);
        std::memcpy(&ts_usec, buffer.data() + 4,  4);
        std::memcpy(&caplen,  buffer.data() + 8,  4);
        buffer.consume(16);
        ...
        if (!buffer.ensure(caplen)) { is_done = true; return false; }

        pkt_data = buffer.data();
        pkt_len  = caplen;

        if (nanosecond_ts) {
            ts_ns = static_cast<std::uint64_t>(ts_sec) * 1000000000ULL + ts_usec;
        } else {
            ts_ns = static_cast<std::uint64_t>(ts_sec) * 1000000000ULL
                   + static_cast<std::uint64_t>(ts_usec) * 1000ULL;
        }

        buffer.consume(caplen);
        return true;
    }
    ...
    void parse_pcapng_shb() {
        ...
        std::uint32_t bom;
        std::memcpy(&bom, buffer.data() + 8, 4);
        ...
        std::memcpy(&block_len, buffer.data() + 4, 4);
        block_len = fix32(block_len);

        if (!buffer.ensure(block_len))
            throw std::runtime_error("truncated pcap-ng SHB block");
        buffer.consume(block_len);
    }

    bool advance_pcapng() {
        while (true) {
            if (!buffer.ensure(8)) { is_done = true; return false; }

            std::uint32_t block_type, block_len;
            std::memcpy(&block_type, buffer.data(),     4);
            std::memcpy(&block_len,  buffer.data() + 4, 4);

            block_type = fix32(block_type);
            block_len  = fix32(block_len);

            if (block_len < 12) { is_done = true; return false; }
            if (!buffer.ensure(block_len)) { is_done = true; return false; }

            const std::byte* block = buffer.data();
    ...
    void parse_pcapng_new_section(const std::byte* block) {
        ...
        if (bom == 0x1a2b3c4d) swap_bytes = false;
        else if (bom == 0x4d3c2b1a) swap_bytes = true;
        interfaces.clear();
    }
    ...
    bool parse_pcapng_epb(const std::byte* block, [[maybe_unused]] std::uint32_t block_len) {
        ...
        std::uint64_t resol = (iface_id < interfaces.size())
                            ? interfaces[iface_id].ts_resol
                            : 1000000ULL;
        ...
        if (resol == 1000000000ULL) {
            ...
            ts_ns = static_cast<std::uint64_t>(
